...
DIALOG_BOARD_RENUM::DIALOG_BOARD_RENUM( PCB_EDIT_FRAME* aParentFrame ) :
                                                 DIALOG_BOARD_RENUM_BASE( aParentFrame ),
...
void    DIALOG_BOARD_RENUM::GetParameters( void )
{
        ...
        if(!m_SortGrid->GetValue( ).ToDouble( &s_savedDialogParameters.SortGrid )) s_savedDialogParameters.SortGrid = DEFAULT_GRID;
        s_savedDialogParameters.SortGrid = s_savedDialogParameters.SortGrid < MINGRID ? MINGRID : s_savedDialogParameters.SortGrid;
}

DIALOG_BOARD_RENUM_CONTINUEABORT::DIALOG_BOARD_RENUM_CONTINUEABORT( wxWindow* parent, wxWindowID id,
                          const wxString& title, const wxPoint& pos, const wxSize& size, long style )
                             : DIALOG_BOARD_RENUM_CONTINUEABORT_BASE( parent, id, title, pos, size, style )
{ }
...
DIALOG_BOARD_RENUM_ABORT::DIALOG_BOARD_RENUM_ABORT( wxWindow* parent, wxWindowID id,
                          const wxString& title, const wxPoint& pos, const wxSize& size, long style )
...
{  s_savedDialogParameters.RenumDialog->ShowMessage( message ); }

void    RenumShowMessage( const char *message )
{  s_savedDialogParameters.RenumDialog->ShowMessage( message ); }

void    RenumShowMessage( const char *message, int arg1  )
{  s_savedDialogParameters.RenumDialog->ShowMessage( wxString::Format( message, arg1 )); }

void    RenumShowMessage( const char *message, std::string& arg1  )
{  s_savedDialogParameters.RenumDialog->ShowMessage( wxString::Format( message, arg1.c_str() )); }
//
void    RenumShowMessage( const char *message, std::string& arg1, std::string& arg2 )
{  s_savedDialogParameters.RenumDialog->ShowMessage( wxString::Format( message, arg1.c_str(), arg2.c_str() )); }
...
int     tmp;
        ...
        if( DescendingFirst ) { tmp = X0; X0 = X1; X1 = tmp; } //If descending, same compare just swap directions
        if( DescendingSecond  ) { tmp = Y0; Y0 = Y1; Y1 = tmp; }
...
std::ofstream tmphandle( FullFileName, std::ios::trunc);             //Open the file for writing
        ...
        if (tmphandle.is_open()) tmphandle << aBuffer;           //Write the buffer
...
{
    if( !s_savedDialogParameters.WriteLogFile ) return;
    LogFile += aMessage;
}

void    LogChangeArray( void )
{
    if( !s_savedDialogParameters.WriteChangeFile  && !s_savedDialogParameters.WriteLogFile ) return;

    ChangeFile = "Change Array\n";
    for( auto Change : ChangeArray )       //Show all the types of refdes
        ChangeFile += Change.OldRefDesString + " -> " + Change.NewRefDes + (Change.Ignore? "* ignored\n" : "\n");
...
{
int i = 1;
std::string Message = "\n\n\nThere are " + std::to_string( RefDesTypes.size()) + " types of reference designations\n";
...
std::string Message = aMessage;
    ...
    Message += "\n*********** Sort on " +
                (std::string)( s_savedDialogParameters.SortOnModules ? "Module" : "Ref Des" )
                    +  " Coordinates *******************"
                        + "\nSort Code " + std::to_string( s_savedDialogParameters.SortDir );
    ...
    for ( auto mod : aModules )
    {
        ...
        Message += " Rounded: X " + std::to_string( mod.x ) + ", Y " + std::to_string(  mod.y );
    }
    LogMessage( Message );
}
...
wxString    GetNewRefDes( wxString aOldRefDes )
{
    for( auto Change : ChangeArray )
        if( aOldRefDes == Change.OldRefDesString ) return( Change.NewRefDes );
...
int             Rounder;
unsigned int    MaxRefDes = 0, iSortGrid = (int) (1000000 * s_savedDialogParameters.SortGrid ) ;
size_t          FirstNum;
...
DIALOG_BOARD_RENUM_PARAMETERS    RenumDialog = s_savedDialogParameters;
    ...
    for ( auto mod : amodules )
    {
        ...
        if( abs( Rounder ) > (iSortGrid/2)) ThisModule.x += ( ThisModule.x < 0 ? -iSortGrid : iSortGrid );

        Rounder = ThisModule.y % iSortGrid;
        ThisModule.y -= Rounder;                                        //X coordinate down to grid
        if( abs( Rounder ) > (iSortGrid/2)) ThisModule.y += ( ThisModule.y < 0 ? -iSortGrid : iSortGrid );
...
std::string  &BackPrefix = s_savedDialogParameters.BackPrefix;
        ...
        || (!FrontPrefix.empty() && RemoveFrontPrefix))
    {
        ...
        RenumShowWarning("**** Overiding and renumbering back sarting at %d ****\n", MaxRefDes);
    ...
    BuildChangeArray( "Top Modules", FrontModules, s_savedDialogParameters.FrontStartRefDes );  //Create the ChangeArray from front
...
std::string     &BackPrefix = s_savedDialogParameters.BackPrefix;
        ...
        for( auto Mod : aModules )          //For each module
        {
            ...
            Change.NewRefDes = Mod.RefDesType + std::to_string( RefDesTypes[i].RefDesCount++ ); //This is the new refdes

            for( auto exclude : ExcludeArray )
                if( exclude == Mod.RefDesType ) Change.Ignore = true;     //If exclude R1 and R1

            if( Change.Ignore ) Change.NewRefDes = Mod.RefDesString;    //Don't change if ignore
            ...
            {	if( Change.Front ) //Now deal with prefixes. This is easy to screw up so I check to make sure no prefix is already there
            	{
                	if((( Change.NewRefDes.substr(0, FrontPrefix.size()) != FrontPrefix )
                			&& AddFrontPrefix )) Change.NewRefDes.insert( 0, FrontPrefix );    //No double dipping
                	if( RemoveFrontPrefix ) Change.NewRefDes.erase( 0., FrontPrefix.size());
            	}
            	else        //Doing the back
            	{
            		if((( Change.NewRefDes.substr(0, BackPrefix.size()) != BackPrefix )
            				&& AddBackPrefix )) Change.NewRefDes.insert( 0, BackPrefix );
            		if( RemoveBackPrefix ) Change.NewRefDes.erase( 0, BackPrefix.size());
            	}
            }
